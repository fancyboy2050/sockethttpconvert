...
public abstract class AZSecurityUtil {
	...
	public String encryptToBase64(String key, String src, String encoding) {
		try {
			return Base64.encodeBase64String(encrypt(key.getBytes(), src
					.getBytes(encoding)));
		} catch (Exception ex) {
			return null;
		}
	...
	public String decryptFromBase64(String key, String src, String encoding) {
		try {
			return new String(
					decrypt(key.getBytes(), Base64.decodeBase64(src)),
					encoding);
		} catch (Exception ex) {
			return null;
//			ex.printStackTrace();
	...
	public String encryptToBase64DefaultEncoding(String key, String src) {
		try {
			return Base64.encodeBase64String(encrypt(key.getBytes(), src.getBytes()));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return null;
